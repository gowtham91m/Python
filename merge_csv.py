...
#pd.set_option('display.max_rows', None) #dilplay all the columns
#pd.set_option('display.expand_frame_repr', None)
def main():
    # get location of script
    # This works only on command prompt. If working on any IDE, set current woking directory
    ...
    recon_path = os.path.join(wd,'reconciliation_files')
    files=os.listdir(data_path)
    recon=os.listdir(recon_path)
    
    # concatenate reconciliation files to single dataframe
    recon_data = 0
    for k in recon:
        if k[-4:]=='.csv':
